...
public class BasiliskEntity extends Monster implements GeoEntity {
    ...
    private PlayState predicate(AnimationState animationState) {
        if(animationState.isMoving()) {
            animationState.getController().setAnimation(RawAnimation.begin().then("animation.basilisk.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;

        }

        animationState.getController().setAnimation(RawAnimation.begin().then("animation.basilisk.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    ...
    private PlayState attackPredicate(AnimationState state) {
        if(this.swinging && state.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            ...
            state.getController().setAnimation(RawAnimation.begin().then("animation.basilisk.attack", Animation.LoopType.PLAY_ONCE));
            this.swinging = false;
        }
